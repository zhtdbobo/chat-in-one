...
export type DotNotationKeyOf<T extends Record<string, any>> = {
    [K in keyof Required<T>]: K extends string ? Required<T>[K] extends Record<string, any> ? K | `${K}.${DotNotationKeyOf<Required<T>[K]>}` : K : never;
}[keyof T];
export type DotNotationValueOf<T extends Record<string, any>, K extends DotNotationKeyOf<T>> = K extends `${infer Head}.${infer Tail}` ? Head extends keyof T ? T[Head] extends Record<string, any> ? Tail extends DotNotationKeyOf<T[Head]> ? DotNotationValueOf<T[Head], Tail> : never : Required<T>[Head] extends Record<string, any> ? Tail extends DotNotationKeyOf<Required<T>[Head]> ? DotNotationValueOf<Required<T>[Head], Tail> | undefined : never : never : never : K extends keyof T ? T[K] : never;
